...
** org-capture setup
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defun my/org-category-files ()
       "Return every org file referenced by `my/org-capture-categories'.
     Includes the shared top-level inbox.org so it is also reachable for
     refiling."
       (let ((files (list (expand-file-name "~/org/src/orgfiles/inbox.org"))))
         (dolist (cat my/org-capture-categories)
           (let ((dir (format "~/org/src/orgfiles/%s/" (nth 2 cat))))
